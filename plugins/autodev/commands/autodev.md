...
│                      └────────┘                                         │
│                                                                          │
│  (*) Smart Merge: task nhỏ (≤2 files) → spec+plan gộp 1 phase          │
│                   task lớn (≥3 files) → spec rồi plan riêng             │
│                                                                          │
...
|-----|--------|----------|--------|-----------|
| 0. Brainstorm | `brainstorming` | orchestrator (trực tiếp) | Codebase context + approach | — |
...
**Smart Merge (v2.3):** Task nhỏ (≤2 files_touched) → gộp spec+plan thành 1 phase `spec_plan_writing` → 1 review `spec_plan_review`. Task lớn (≥3 files) → tách spec/plan riêng như trên.

### Bảng Chuyển Trạng Thái

```
pending → brainstorming                   (khi task bắt đầu — orchestrator explore codebase)
brainstorming → spec_writing              (task lớn: ≥3 files_touched)
brainstorming → spec_plan_writing         (task nhỏ: ≤2 files_touched — gộp spec+plan)

// --- Task lớn: spec và plan riêng ---
...
// --- Task nhỏ: spec+plan gộp ---
spec_plan_writing → spec_plan_review      (author viết spec+plan gộp)
spec_plan_review → spec_plan_writing      (reviewer trả "issues", loop < max)
spec_plan_review → implementing           (reviewer trả "approved")
...
1. **Brainstorm** — Orchestrator explore codebase (Section 8.0) → hiểu context trước
2. **Parse requirement** — Tách yêu cầu thành các tasks độc lập
3. **Detect dependencies** — Phân tích xem tasks có phụ thuộc nhau không (VD: "thêm caching" phụ thuộc "thêm rate limiting")
4. **Khai báo `files_touched`** — Mỗi task liệt kê danh sách files dự kiến sẽ sửa
5. **Smart merge decision** — Per task:
   - `files_touched.length <= 2` → `smart_merge = true` → gộp spec+plan
   - `files_touched.length >= 3` → `smart_merge = false` → tách spec/plan riêng
6. **Gán parallel groups** — So sánh `files_touched` giữa các tasks:
  ...
  "tasks": [
    {
      ...
      "status": "pending|brainstorming|spec_writing|spec_review|spec_plan_writing|spec_plan_review|plan_writing|plan_review|implementing|code_review|pr_created|pr_review|completed|failed|cancelled|blocked|split",
      "smart_merge": false,           // v2.3: true = spec+plan gộp (task ≤2 files)
      "phase_loop_count": 0,
      "depends_on": [],
      ...
      "brainstorm": null,              // v2.3: { codebase_context, approach, files_identified, risks, smart_merge, clarifications }
      "artifacts": {
        "spec": null,           // null | "path"
        "plan": null,           // null | "path"
      ...
      "cross_model": {                // OPTIONAL — chi co khi cross-model enabled
        ...
        "reviewer_model": null,       // "gpt-5.4" | null
...
### 8.0 brainstorm (v2.3) — Orchestrator trực tiếp, KHÔNG spawn teammate

**Isolation:** không | **Mode:** orchestrator tự chạy
**Tools:** Read, Grep, Glob, Bash (git log, git diff)

Brainstorm phase chạy BỞI ORCHESTRATOR (không spawn teammate). Orchestrator tự explore codebase và chuẩn bị context cho author.

**Flow:**

```
1. EXPLORE CODEBASE — hiểu code hiện tại
   - Grep/Glob tìm files liên quan đến request
   - Đọc files chính để hiểu architecture
   - git log --oneline -20 xem recent changes
   - Identify patterns, conventions, test setup

2. CLARIFY (nếu cần) — hỏi user
   - Request mơ hồ? → hỏi user trước khi tiếp
   - Nhiều approach khả thi? → đề xuất + hỏi preference
   - Scope quá lớn? → đề xuất tách nhỏ

3. ASSESS — đánh giá task
   - Estimate files_touched → quyết định smart merge (≤2 → gộp, ≥3 → tách)
   - Identify risks, constraints, dependencies
   - Check overlap với workflows đang chạy (Section 17)

4. OUTPUT — lưu context vào state
   state.tasks[].brainstorm = {
     "codebase_context": "Tóm tắt architecture liên quan...",
     "approach": "Proposed approach...",
     "files_identified": ["src/auth/index.ts", ...],
     "risks": ["Risk 1...", ...],
     "smart_merge": true/false,  // true = gộp spec+plan
     "clarifications": []        // câu hỏi đã hỏi user (nếu có)
   }
```

**Brainstorm output được truyền vào author prompt** — author bắt đầu với context thay vì blank slate.
...
Review cũng gộp: 1 reviewer check cả spec lẫn plan trong 1 pass.
Sau approved → chuyển thẳng `implementing` (skip plan_writing + plan_review).
...
## Brainstorm Context (từ phase 0)
{task.brainstorm.codebase_context}
Approach: {task.brainstorm.approach}
Files identified: {task.brainstorm.files_identified}
Risks: {task.brainstorm.risks}

## Bối cảnh

- Brainstorm context ở trên là điểm khởi đầu — explore thêm nếu cần
- Assess the blast radius of changes

## Phase-specific instructions

### Khi SPEC+PLAN GỘP (spec_plan_writing — task nhỏ ≤2 files):
- Viết 1 file gộp chứa cả What (spec) và How (plan)
- Pattern: {specs_dir}/YYYY-MM-DD-<topic>-design.md
- File gộp chứa: ## What (Spec) → ## How (Plan) → ## Decisions
- Commit: "docs: add {slug} design"
- Sau review approved → chuyển thẳng implementing

### Khi SPEC (spec_writing — task lớn ≥3 files):
...
- `{slug}` → task slug
- `{reviewer_model}` → model ID cua reviewer (VD: "gpt-5.4") (v2.1)
...
agents:
  role_mapping:
    ...
    reviewer: gpt-5.4            # unified reviewer — 1 model xuyên suốt spec/plan/code/PR review
    escalation: gemini-2.5-pro
```

**Smart model selection:**
- **Task size nhỏ** (1-2 files) → có thể dùng model nhẹ (haiku)
- **Task size lớn** (3+ files) → dùng model mạnh (opus, gpt-5.4)
...
```
🔵 ▸ [HH:MM:SS] wf_id/slug cross-model review: writer=opus, reviewer=gpt-5.4
...
4. BRAINSTORM + PHÂN TÁCH TASKS (Section 8.0, 5)
   → LOG: 🔵 ▸ [{HH:MM:SS}] Brainstorm — exploring codebase...
   └── Explore codebase: Grep/Glob files liên quan, đọc architecture
   └── Clarify với user nếu request mơ hồ
   └── Parse requirement → tasks
   └── Khai báo files_touched cho mỗi task
   └── Smart merge decision: ≤2 files → gộp spec+plan, ≥3 → tách
   └── Gán parallel groups
   → LOG: Hiển thị decomposition box (có smart_merge status) → chờ user xác nhận
...
9. CHO MỖI PARALLEL GROUP (tuần tự):
   ...
   │       │
   │       ├── PHA BRAINSTORM (Section 8.0):
   │       │   ├── 🔵 ▸ [{HH:MM:SS}] {wf_id}/{slug} brainstorming...
   │       │   ├── Orchestrator explore codebase cho task cụ thể
   │       │   ├── Lưu brainstorm context vào state
   │       │   └── Quyết định smart_merge (≤2 files → gộp, ≥3 → tách)
   │       │
   │       ├── PHA SPEC+PLAN (smart_merge=true → gộp, false → tách):
   │       │   │
   │       │   ├── [NẾU smart_merge=true — task nhỏ]:
   │       │   │   ├── 🔵 ▸ [{HH:MM:SS}] {wf_id}/{slug} spec_plan_writing (gộp)...
   │       │   │   ├── Cache lookup → HIT? 🟣 CACHE HIT, skip : dispatch
   │       │   │   ├── Spawn author viết spec+plan gộp (Section 8.0.1)
   │       │   │   ├── 🔵 ▸ [{HH:MM:SS}] {wf_id}/{slug} spec_plan_review...
   │       │   │   ├── Spawn reviewer review spec+plan gộp
   │       │   │   ├── Loop nếu cần (max 3)
   │       │   │   ├── 🟢 ▸ [{HH:MM:SS}] {wf_id}/{slug} spec+plan APPROVED
   │       │   │   └── 🟣 checkpoint → chuyển thẳng implementing
   │       │   │
   │       │   └── [NẾU smart_merge=false — task lớn]:
   │       │       ├── 🔵 ▸ [{HH:MM:SS}] {wf_id}/{slug} spec_writing...
   │       │       ├── Cache lookup → HIT? 🟣 CACHE HIT, skip : dispatch
   │       │       ├── Spawn/SendMessage spec-writer (Section 9.3/9.4)
   │       │       ├── 🔵 ▸ [{HH:MM:SS}] {wf_id}/{slug} spec_review...
   │       │       ├── Spawn/SendMessage reviewer
   │       │       ├── Loop nếu cần (max 3)
   │       │       ├── 🟢 ▸ [{HH:MM:SS}] {wf_id}/{slug} spec APPROVED
   │       │       ├── 🟣 checkpoint
   │       │       ├── 🔵 ▸ [{HH:MM:SS}] {wf_id}/{slug} plan_writing...
   │       │       ├── Cache lookup → HIT? skip : dispatch
   │       │       ├── SendMessage plan-writer
   │       │       ├── 🔵 ▸ [{HH:MM:SS}] {wf_id}/{slug} plan_review...
   │       │       ├── Loop nếu cần (max 3)
